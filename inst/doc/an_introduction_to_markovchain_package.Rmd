...
output: if (rmarkdown::pandoc_version() < "2.7") function(...) { rmarkdown::pdf_document(template = "./template.tex", ...) } else function(...) { bookdown::pdf_book(base_format = rticles::jss_article, ...) } 
vignette: >
...
\begin{table}[h]
  ...
  \code{entropyRate} & entropy rate of an irreducible Markov chain. \\
  \code{meanFirstPassageTime} & matrix or vector of mean first passage times. \\
  \code{kemenyConstant} & stationary-weighted mean hitting time for an irreducible chain. \\
  ...
  \code{is.stochasticallyMonotone} & checks whether a DTMC (or a Matrix) is stochastically monotone. \\
  ...
  \hline
  \end{tabular}
\caption{\pkg{markovchain} methods: probabilistic operations.}
...
solving the eigenvalues and eigenvectors of the matrix and taking real parts (since if $u + iv$ is a eigenvector, for the matrix $P$, then $Re(u + iv) = u$ and $Im(u + iv) = v$ are eigenvectors) and normalizing by the vector sum, this carries some concerns:

1. If $u, v \in \mathbb{R}^n$ are linearly independent eigenvectors associated with the eigenvalue $1$, $u + iv$, $u + iu$ are also linearly independent eigenvectors, and their real parts coincide. Clearly if we took real parts, we would be losing an eigenvector, because we cannot know in advance if the underlying algorithm to compute the eigenvectors is going to output something similar to what we described. We should be agnostic to the underlying eigenvector computation algorithm.

2. Imagine the identity $P$ of dimensions $2 \times 2$. Its eigenvectors associated to the $1$ eigen value are $u = (1, 0)$ and $v = (0, 1)$. However, the underlying algorithm to compute eigenvectors could return $(1, -2)$ and $(-2, 1)$ instead, that are linear combinations of the aforementioned ones, and therefore eigenvectors. Normalizing by their sum, we would get: $(-1, 2)$ and $(2, -1)$, which obviously are not probability measures. Again, we should be agnostic to the underlying eigen computation algorithm.

3. Algorithms to compute eigenvalues and eigenvectors are computationally expensive: they are iterative, and we cannot predict a fixed number of iterations for them. Moreover, each iteration takes $\mathcal{O}(m^2)$ or $\mathcal{O}(m^3)$ algorithmic complexity, with $m$ the number of states.
...
For a stationary first-order Markov chain, the entropy rate is the conditional
entropy of the next state given the current state:

\[
H(X) = H(X_{t+1}\mid X_t)
     = -\sum_i \pi_i\sum_j p_{ij}\log_b p_{ij},
\]

where $\pi$ is the stationary distribution and $0\log(0)$ is interpreted as
zero. With $b=2$ the result is measured in bits per transition; with $b=e$ it
is measured in nats per transition \citep{coverThomas2006}. The method
`entropyRate` uses the unique stationary distribution of a finite irreducible
chain. Aperiodicity is not required; reducible chains are rejected because the
result can depend on the chosen stationary distribution
\citep{strelioff2007inferring}.

```{r entropy-rate-weather}
entropyRate(mcWeather)
entropyRate(mcWeather, base = exp(1))
```

A deterministic chain has entropy rate zero, whereas a chain whose rows are all
uniform on $n$ states has the maximum value $\log_b(n)$.
...
For a finite irreducible chain with stationary distribution $\pi$, let
$m_{ij}$ denote the mean time to hit state $j$ from state $i$, using
$m_{ii}=0$. Kemeny's constant is

\[
K=\sum_j \pi_j m_{ij}.
\]

Remarkably, $K$ does not depend on the starting state $i$. Equivalently, if
$Z=(I-P+\mathbf{1}\pi^T)^{-1}$, then $K=\operatorname{tr}(Z)-1$
\citep{kemeny1960finite}. The function `kemenyConstant` uses this formula and
requires irreducibility, but not aperiodicity. If mean *return* times
$m_{jj}=1/\pi_j$ are placed on the diagonal instead, the weighted sum is
$K+1$.
...
The second result has one entry for each possible starting state; apart from
numerical rounding, all entries equal Kemeny's constant.
...
Thus, the expected time to absorption is three steps from states 2 or 4 and four steps from state 3.

### Fundamental matrix

For an absorbing chain, let $Q$ be the transition submatrix among transient
...
Its $(i,j)$ entry is the expected number of visits to transient state $j$ before
absorption when starting from transient state $i$, including the initial visit.
The expected absorption-time vector is $N\mathbf{1}$, while absorption
probabilities are obtained from $NR$, where $R$ contains transitions from
transient to absorbing states [@GrinsteadSnell].

```{r fundamental-matrix-drunkard}
...
For states $i$ and $j$, the hitting probability $h_{ij}$ is the probability of
ever visiting $j$ when the chain starts in $i$. For $i = j$, the package uses
the return probability after at least one transition, rather than the trivial
time-zero probability. The values are the minimal non-negative solution of the
...
`hittingProbabilities` first analyses the directed graph of positive transitions.
This identifies probabilities that are structurally zero or one, without
discarding very small positive transitions. Remaining probabilities are computed
from a convergent Neumann series using a relative residual stopping rule. This is
important for nearly absorbing chains: a small transition probability may imply
a long waiting time, but the probability of eventually taking that transition
can still be exactly one.
...
For the irreducible finite chain `mcWeather`, every state is eventually visited
...
A transition matrix $P$ is stochastically monotone if the probability of moving to higher states is non-decreasing with respect to the starting state. 
In other words, row $i$ is stochastically dominated by row $j$ whenever $i < j$. 
This property is fundamental in many applied models, such as credit ratings or social mobility, 
because it guarantees that the Markov chain preserves the initial ordering of states over time.

For instance, in an income mobility model, if you start in a higher income class, 
your probability of moving to or staying in a high income class should ideally be greater than or equal to someone starting in a lower income class.

The `is.stochasticallyMonotone` method allows verifying this property for any `markovchain` object:

```{r stochasticMonotonicity}
...
```{r stochasticMonotonicityNonMonotone}
...
Two advanced parameters are available to the `rmarkovchain` method which help users decide which implementation to use. There are four options available : \proglang{R}, \proglang{R} in parallel, \proglang{C++} and \proglang{C++} in parallel. Two boolean parameters `useRcpp` and `parallel` will decide which implementation will be used. Default is \code{useRcpp = TRUE} and \code{parallel = FALSE} i.e. \proglang{C++} implementation. The \proglang{C++} implementation is generally faster than the `R` implementation. If you have multicore processors then you can take advantage of `parallel` parameter by setting it to `TRUE`. When both `useRcpp = TRUE` and `parallel = TRUE` the parallelization has been carried out using \pkg{RcppParallel} package \citep{pkg:RcppParallel}.

## Lumpability and aggregation of states

In some applications the original state space is too detailed, and it is useful
to aggregate several micro-states into a smaller number of macro-states. A Markov
chain is said to be strongly lumpable with respect to a partition of the state
space if, for every pair of macro-states, the total probability of moving from
a micro-state in the source block to the destination block is the same for all
micro-states in the source block \citep{kemeny1960finite}. Under this condition,
the aggregated process is again Markovian and its transition matrix is well defined.

The function `is.lumpable` checks exact strong lumpability for a user-supplied
partition. The function `lump` returns the aggregated Markov chain. By default,
`lump` requires exact lumpability; setting `force = TRUE` performs an approximate
weighted aggregation.

The following example is the classical Land of Oz weather chain. We aggregate
`rainy` and `snowy` into a single `Bad_Weather` macro-state, leaving `nice` as a
separate macro-state.
...
The lumped chain has two states. From either `rainy` or `snowy`, the probability
of moving to `nice` is 0.25, so the transition probability from `Bad_Weather` to
`Nice_Weather` is also 0.25. The remaining probability, 0.75, stays inside the
`Bad_Weather` macro-state.

If the exact lumpability condition is not satisfied, `lump` stops by default:

```{r lumpability-nonexact}
P_bad <- matrix(
  c(0.5, 0.3, 0.2,
    0.7, 0.2, 0.1,
    0.1, 0.4, 0.5),
...
For exploratory analyses, approximate lumping can be forced:

```{r lumpability-forced}
lump(mcBad, partition_bad, force = TRUE)
```

When `force = TRUE`, the aggregated transition probabilities are computed using
stationary weights when available. If a macro-state has zero stationary mass, an
arithmetic average over its micro-states is used as a fallback. This produces a
valid coarse transition matrix, but it should be interpreted as an approximation:
the aggregated process is not guaranteed to be exactly Markovian unless the
strong lumpability condition holds.

The helper `autoLump` provides a heuristic way to discover macro-states using
spectral clustering. It should be regarded as an exploratory approximation, not
as a test of exact lumpability.

```{r autolump-example, eval=FALSE}
...
To keep the vignette fast, the example uses a small transition matrix. In larger
applications, the automatically discovered partition should be inspected and
validated substantively before being used for modelling or forecasting.

## Estimation

A time-homogeneous Markov chain can be fitted from observed state sequences. The current version of \pkg{markovchain} implements four fitting methods: maximum likelihood (MLE), MLE with Laplace smoothing, bootstrap estimation and maximum a posteriori (MAP) estimation.

Equation \ref{eq:MLE} shows the maximum likelihood estimator of the transition probability $p_{ij}$. Here $n_{ij}$ is the number of observed transitions $X_t=s_i, X_{t+1}=s_j$ and $n_i = \sum_{u=1}^k n_{iu}$ is the number of observed outgoing transitions from state $s_i$:

\begin{equation}
{\hat p^{MLE}}_{ij} = \frac{n_{ij}}{n_i}.
\label{eq:MLE}
\end{equation}

For a fixed origin state $s_i$, the counts $(n_{i1}, \ldots, n_{ik})$ are treated as a multinomial sample with row total $n_i = \sum_{u=1}^k n_{iu}$. The `standardError` returned for each marginal transition probability is therefore the usual binomial standard error in Equation \@ref(eq:SE).

\begin{equation}
SE_{ij} = \sqrt{\frac{{\hat p^{MLE}}_{ij}(1-{\hat p^{MLE}}_{ij})}{n_i}}
\label{eq:SE}
\end{equation}
...
The Laplace smoothing approach is a smoothed version of the MLE in which each count $n_{ij}$ is replaced by $n_{ij}+\alpha$ (Equation \ref{eq:LAPLACE}). The smoothing parameter $\alpha$ must be non-negative; $\alpha=0$ gives the unsmoothed MLE.
...
The confidence interval option is enabled by default; set `confint = FALSE` to reduce computation time when intervals are not needed. Both MLE and Laplace fitting are based on `createSequenceMatrix`, which returns the raw transition-count matrix.
...
If a state is observed only as the last element of a sequence, or is supplied through `possibleStates` but has no observed outgoing transition, its row has zero total count. In this case the point estimate follows the package convention of assigning a uniform row, $p_{\beta j}=1/k$. For MLE confidence intervals, such a row carries no empirical information, so the corresponding cell-wise intervals are uninformative.
...
A bootstrap estimation approach is also available to assess the variability of $\hat p_{ij}$. The implementation in \pkg{markovchain} proceeds as follows:

  1. generate bootstrap state sequences using the transition probabilities estimated from the original data. The number of bootstrap samples is controlled by the `nboot` argument of `markovchainFit`;
  2. apply MLE estimation to each bootstrap sequence; the bootstrap estimates are saved in the `bootStrapSamples` component of the returned list;
  3. compute the bootstrap transition estimate as the row-normalized average of the bootstrap MLE estimates. The reported `standardError` is the empirical standard deviation of the bootstrap estimates for each transition probability.
...
The bootstrapping process can be done in parallel thanks to \pkg{RcppParallel} package \citep{pkg:RcppParallel}. The parallelized implementation is recommended when the data sample size or the required number of bootstrap runs is high.
...
Confidence matrices of estimated parameters are available as well. The `confidenceInterval` component contains two matrices, `lowerEndpointMatrix` and `upperEndpointMatrix`. The confidence level is 0.95 by default and can be changed through the `confidencelevel` argument of `markovchainFit`.

For MLE fits, \pkg{markovchain} reports cell-wise Wilson score intervals for the marginal transition probabilities \citep{brown2001interval}. Compared with the simple Wald interval $\hat p_{ij} \pm z SE_{ij}$, Wilson intervals have better small-sample behaviour and do not collapse to a zero-width interval when a transition is never observed or always observed. With $z = \Phi^{-1}(1 - \alpha/2)$, $n_i = \sum_u n_{iu}$, $\hat p_{ij}=n_{ij}/n_i$, and confidence level $1-\alpha$, the interval is

\begin{align}
LowerEndpoint_{ij} &= \frac{\hat p_{ij} + z^2/(2n_i) - z\sqrt{\hat p_{ij}(1-\hat p_{ij})/n_i + z^2/(4n_i^2)}}{1+z^2/n_i}, \label{eq:CIL} \\
UpperEndpoint_{ij} &= \frac{\hat p_{ij} + z^2/(2n_i) + z\sqrt{\hat p_{ij}(1-\hat p_{ij})/n_i + z^2/(4n_i^2)}}{1+z^2/n_i}.
\label{eq:CIU}
\end{align}

If no outgoing transition is available for a state, the point estimate follows the package convention of a uniform row, while the interval is left maximally uninformative. These intervals are cell-wise marginal intervals, not simultaneous multinomial confidence regions for a full row or matrix. For simultaneous multinomial intervals, use `multinomialConfidenceIntervals`. For bootstrap fits, intervals use the empirical standard deviation of the bootstrap transition-probability estimates and the two-sided normal quantile corresponding to `confidencelevel`. For MAP fits, credible intervals are equal-tailed beta posterior intervals, obtained from the inverse beta distribution \citep{strelioff2007inferring}.
...
The $n$-step forward predictions can be obtained using the `predict` methods explicitly written for `markovchain` and `markovchainList` objects. The prediction is the mode of the conditional distribution of $X_{t+1}$ given $X_{t}=s_{j}$, being $s_{j}$ the last realization of the DTMC (homogeneous or semi-homogeneous).
...
In this section, we describe the statistical tests: assessing the Markov property (`verifyMarkovProperty`), the order (`assessOrder`), the time-homogeneity (`assessStationarity`) of a Markov chain sequence, and the divergence tests for empirically estimated transition matrices (`verifyEmpiricalToTheoretical`, `verifyHomogeneity`). Most of such tests are based on the $\chi ^2$ statistics; the Pearson and likelihood-ratio (G) versions of each test are provided, together with a Monte Carlo option (`method = "simulation"`) that avoids the asymptotic $\chi^2$ approximation altogether by simulating the null distribution of the G statistic directly from the fitted model. Test results are returned as objects of class `htest`, following R's standard convention for hypothesis tests, so that they print, and can be manipulated, like the output of `stats::chisq.test`. Relevant references are \cite{kullback1962tests} and \cite{anderson1957statistical}.

All such tests have been designed for small samples, since it is easy to detect departures from the Markov property as long as the sample size increases. The Monte Carlo option is particularly useful in this regime, since the asymptotic $\chi^2$ approximation can be unreliable when expected counts are small; the simulated $p$-value is computed as $(1+\#\{G^*\ge G\})/(B+1)$ over $B$ parametric replicates drawn under the null hypothesis, which is a finite-sample-valid Monte Carlo test \citep{davisonHinkley1997}. The accuracy of the statistical inference functions for very small samples is nonetheless an area of ongoing work.

### Assessing the Markov property of a Markov chain sequence

The `verifyMarkovProperty` function verifies whether the Markov property holds for the given chain. The test implemented in the package looks at triplets of successive observations. If $x_1, x_2, \ldots, x_N$ is a set of observations and $n_{ijk}$ is the number of times $t$ $\left(1 \le t \le N-2 \right)$ such that $x_t=i, x_{t+1}=j, x_{t+2}=k$, then, if the Markov property holds, $n_{ijk}$ is expected to be close to $n_{ij}\hat p_{jk} = n_{ij}\,n_{jk}/n_j$, where $n_{ij}=\sum_k n_{ijk}$, $n_{jk}=\sum_i n_{ijk}$ and $n_j = \sum_k n_{jk}$. The corresponding Pearson and likelihood-ratio statistics are

$$
X^2=\sum_{i}\sum_{j}\sum_{k}\frac{(n_{ijk}-n_{ij}\hat{p}_{jk})^2}{n_{ij}\hat{p}_{jk}}, \qquad
G^2=2\sum_{i}\sum_{j}\sum_{k} n_{ijk}\log\frac{n_{ijk}}{n_{ij}\hat{p}_{jk}},
$$

both asymptotically $\chi^2(q)$ under the null hypothesis \citep{anderson1957statistical}. The number of degrees of freedom $q$ is obtained by summing, over each present state $j$, the degrees of freedom of the corresponding (past $\times$ future) contingency table restricted to the past and future states that were actually observed together with $j$: $q=\sum_j (s_j-1)(r_j-1)$, where $s_j$ is the number of past states $i$ with $n_{ij}>0$ and $r_j$ is the number of future states $k$ with $n_{jk}>0$.
...
Burkett, Scherer and Todd (2019) \citep{burkett2019recurrent} study recurrent and persistent financial-market states. One of their published tests considers the transition into state 1 and reports the following counts, for which a standard Pearson test of independence gives a non-significant result:
...
The published contingency table gives a non-significant Pearson test ($\chi^2\approx0.64$, 3 degrees of freedom, $p\approx0.89$). It is included as an independent literature example of a non-rejection; it is not claimed to be numerically identical to `verifyMarkovProperty`, since the published test is a cell-wise test for a particular transition rather than the package's global first-order Markov-property test.

### Assessing the order of a Markov chain sequence

The `assessOrder` function checks whether the given chain is of first order or of second order. For each possible present state, we construct a contingency table of the frequency of the future state for each past-to-present state transition, as shown in Table \ref{tab:order}, and performs a Pearson $\chi^2$ test of independence between past and future conditionally on the present state, summing the resulting statistics over all present states.
...
As for `verifyMarkovProperty`, degrees of freedom are summed over present states
using only the past and future states actually observed together with that present
state, $q=\sum_j (s_j-1)(r_j-1)$, rather than a fixed $k(k-1)^2$; the two coincide
whenever every state is observed as both a past and a future state for every present
state, and only differ -- correctly reducing $q$ -- when some transitions are unobserved.
If the p-value is greater than the given significance level, we cannot reject the
hypothesis that the sequence is of first order.
...
### Assessing the stationarity and time-homogeneity of a Markov chain sequence

The `assessStationarity` function assesses whether the transition probabilities of the given chain change over time, i.e. whether
...
For each state, the observed sequence is split into `nblocks` contiguous blocks and a (block $\times$ destination state) contingency table of one-step-ahead transition counts is built, as shown in Table \ref{tab:stationarity}; a Pearson $\chi^2$ test of homogeneity across blocks is then performed and the resulting statistics summed across states, with degrees of freedom adapted, state by state, to the blocks and destinations that were actually observed.
...
Tran and Carmichael (2012) \citep{tranCarmichael2012} apply likelihood-ratio tests to transition probabilities in a real contractor-payment application, and provide an example where the null hypothesis of time-homogeneous transition probabilities is rejected. Their application also motivates explicit handling of *structural zeros*, i.e. transitions that are impossible under the state-space model, as opposed to *sampling zeros*, i.e. transitions that are possible but were simply not observed. The package allows structural zeros to be specified through the `structural.zeros` argument, so that they are excluded from the contingency tables rather than treated as evidence of homogeneity:
...
This section discusses tests developed to verify whether: 
  ...
  2. Two or more empirical transition matrices belong to the same DTMC. 

The first test is implemented by the `verifyEmpiricalToTheoretical` function. Denoting by $f_{ij}$ the raw empirical transition count and by $P_{ij}$ the theoretical (fully specified) transition probability, expected counts are constructed row-wise as $E_{ij}=f_{i+}P_{ij}$, where $f_{i+}=\sum_j f_{ij}$, and

$$
G^2 = 2\sum_{i=1}^{r}\sum_{j=1}^{r}f_{ij}\ln\frac{f_{ij}}{f_{i+}P_{ij}} \sim \chi^2\left ( \textstyle\sum_i (k_i - 1) \right ),
$$

where $k_i$ is the number of states reachable with positive probability from $i$ under the theoretical matrix; since $P$ is fully specified rather than estimated, no degrees of freedom are lost to parameter estimation. The following example is taken from \cite{kullback1962tests}:
...
Kullback et al. report $G^2=6.551795$, $df=6$ and $p=0.3642899$ for this table. The historical `markovchain` implementation reproduced this published value by using column totals when constructing expected counts. For the row-wise multinomial null implemented by the current package -- the standard construction for this kind of test, since each row of a transition matrix is itself a separate multinomial sample -- expected counts are $E_{ij}=f_{i+}P_{ij}$, which gives $G^2=6.518384$ with the same $df=6$ and the same non-rejection of the null hypothesis. The small numerical discrepancy with the published value is therefore a correction of the historical implementation rather than a change in the inferential conclusion.

The second test is implemented by the `verifyHomogeneity` function, inspired by \cite[section~9]{kullback1962tests}. Assuming that $i=1,2, \ldots, s$ DTMC samples are available and that the cardinality of the state space is $r$, it verifies whether the $s$ chains belong to the same unknown one. Pooling counts across samples for each state $i$ and comparing each sample's destination distribution to the pooled one, the resulting statistic is asymptotically $\chi^2$ with degrees of freedom summed, state by state, over the samples and destinations actually observed for that state. The following example generates two independent sequences from the same chain, for which the homogeneity hypothesis should not be rejected:
...
For sparse data, the asymptotic $\chi^2$ approximation underlying any of the tests above should be interpreted with care; the `method = "simulation"` option provides a Monte Carlo alternative that does not rely on the asymptotic approximation.
...
This is the memoryless property that only the exponential random variable exhibits. Therefore, the exponential distribution is therefore the relevant distribution, and each state $s \in S$ has an exponential holding parameter $\lambda(s)$. Since $\mathrm{E}T_x = \frac{1}{\lambda(x)}$, the higher the rate $\lambda(x)$, the smaller the expected time of transitioning out of the state $x$.
...
The credible intervals are constructed as equal-tailed intervals by computing the inverse of the beta integral. 
...
Let us consider 100 simulated life trajectories for a healthy individual aged 80. We can compute the expected time the individual is disabled, starting from the active state at age 80.
