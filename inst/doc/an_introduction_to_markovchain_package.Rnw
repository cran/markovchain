...
fields analyzed by means of the \pkg{markovchain} package. For continuous time Markov chains, refer to the CTMC vignette which is a part of the package.
...
The $\phi$-divergence test \cite{pardo2005statistical} between two empirically estimated transition matrices $m1$ and $m2$ is given by

\begin{equation}
T^{\phi}_n(m_1, m_2, mc)=\frac{2n}{\phi^{''}(1)} \sum_{i=1}^{M} \frac{v_{i*}}{n} \sum_{j=1}^{M} p^0(i,j) \phi \left(\frac{m_1(i,j)}{m_2(i,j)} \right)
\label{eq:phidivtest}
\end{equation}
