...
\author{Giorgio Alfredo Spedicato, Ph.D C.Stat ACAS \And \\
        Mirko Signorelli, M.Sc. Statistics and economics}
...
In particular, discrete Markov chains permit to model the transition probabilities between discrete states by the aid of matrices.
Various R packages deals with Markov chains processes and their applications:
\pkg{msm} \citep{msmR} handle Multi-State Models for Panel Data, \pkg{mcmcR}
\citep{mcmcR} is only one of the many package 
that implements Monte Carlo Markov Chain approach for 
estimating models' parameters, \pkg{hmm} fits hidden markov models taking 
into account covariates. R statistical environment \citep{rSoftware} seems to lack a simple
R package that coherently defines S4 classes for discrete Markov chains and 
that allows to perform probabilistic analysis, statistical inferences and
applications. \pkg{markovchain} \citep{markovchainR} aims to offer greater
flexibility in handling discrete time Markov chains. The paper is structured as follows: 
Section~\ref{sec:mathematic} briefly revies mathematic and 
definitions on discrete Markov chains, Section~\ref{sec:examples} shows 
applied example of discrete Markov chains in various fields.
...
A discrete-time Markow chain is a sequence of random variables $X_{1,}X_{2},X_{3},\ldots$
characterized by memorylessness property (also known as Markov property, see
Equation~\ref{eq:markovProp}), that is that the next state of $X_{n+1}$ depends only by the
current state of $X_{n}$ and not by the events that preceded it.
...
holds, that implies no change in the underlying transition probabilities as time
goes on.
\begin{equation}
...
from state $s_{i}$ to state $s_{j}$ shown in Equation~\ref{eq:trProp} is called
...
A group of one or more communicating states generates a communicating class. A
Markov chain is composed by one or more communicating classes. A communicating class is 
said to be closed if no states outside of the class can
be reached from any state inside it.\\
...
Non-homogeneous markov chains can be created with the aid of \code{markovchainList} object. 
The example that follows arises from Health Insurance, where the costs associated to patients in a Continuous Care Health Community (CCHC) are modelled by a non-homogeneous Markov Chain, since the 
transition probabilities can change by year. Methods explicitely written for
...
determining the number of underlying \code{markovchain} objects contained therin in advance.
...
Finally, the \code{markovchain} package contains some data sets found in
literature on which discrete Markov chain models have been applied.
Table~\ref{tab:datasets} lists data set bundled within the current release of
the package.
...
\hline
\end{tabular}
\caption{\pkg{markovchain} datasets.}
\label{tab:datasets}
\end{table}

\section{Statistics with markovchain objects}
...
chains are identified by following steps:
\begin{enumerate}
...
\begin{table}[h]
  ...
  \code{markovchainFit} & function to return fitten markov chain for a given sequence.\\
   ...
   \code{predict} & method to predict from \code{markovchain} or
...
Michael Cole to having signaled it), since it yields to a row of zero (no sample to estimate the conditional
...
of the monthly transition matrix is obtained in order to describe the monthly progression of CD4-cell 
counts of HIV infected subjects starting from six month follow-up data.\\
