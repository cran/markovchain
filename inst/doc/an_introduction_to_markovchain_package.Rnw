...
\author{Giorgio Alfredo Spedicato, Mirko Signorelli}
...
discrete time Karkov chains, homogeneous and inhomogeneous.
The S4 classes classes and methods to handle and analyze discrete time Markov
chain processes will be presented.
Applied examples in Economic, Finance and Natural Sciences will follow.}
\Keywords{discrete time Markov chains, transition matrices}
\Plainkeywords{discrete time Markov chains, transition matrices} %% without formatting
...
\Address{
  ...
  Via Cesare Battisti 241, Padova 35121 Italy\\
  E-mail: \email{signorelli@stat.unipd.it}
}
...
Various R packages deal with models that are based on Markov chains:
\pkg{msm} \citep{msmR} handle Multi-State Models for panel data, \pkg{mcmcR}
\citep{mcmcR} is one among many packages that implement Monte Carlo Markov Chain approach that is widely used to estimate parameters of parametric statistical models.  Hidden Markov models with covariates are fit with package \pkg{hmm}, whilst \pkg{mstate} fits 
Multi-State Models based on Markov chains for survival analysis \citep{mstateR}. Nevertheless, the \proglang{R} statistical environment \citep{rSoftware} seems to lack a simple package that coherently defines S4 classes for discrete Markov chains and that allows to perform probabilistic analysis, statistical inferences and
applications. For the sake of completeness, \pkg{markovchain} is not the first package specifically dedicated to DTMC analysis, being \pkg{DTMCPack} the first one, \cite{DTMCPackR}. Notwithstanding, \pkg{markovchain} package \citep{markovchainR} aims to offer
greater flexibility in handling discrete time Markov chains than other existing
...
By the way, other scientifical softwares provide functions specifically designed
to analyze DTMC, as \proglang{Mathematica} 9 \citep{mathematica9} for example.\\

The paper is structured as follows:
Section~\ref{sec:mathematic} briefly reviews mathematics and 
definitions regarding DTMC, Section~\ref{sec:structure}
discusses on how to handle and manage Markov chain objects within the package,
...
Section~\ref{sec:applications} presents applied examples of DTMC in various
fields using the \pkg{markovchain} package.

Finally, the \pkg{markovchain} package depends on following \proglang{R} packages: \pkg{expm} \citep{expmR} to perform efficient matrices powers; \pkg{igraph} \citep{pkg:igraph} to perform pretty plotting of \code{markovchain} objects and \pkg{matlab} \citep{pkg:matlab} that contains functions for matrix management and calculations that emulate those within \proglang{Matlab} environment.

\section{Markov chains mathematic review}\label{sec:mathematic}

\subsection{General Definitions}

A DTMC consists in a sequence of random variables $X_{1}, X_{2},X_{3},\ldots$
characterized by the property of memorylessness (also known as Markov property,
see Equation~\ref{eq:markovProp}). It states that the distribution of
forthcoming state (of $X_{t+1}$ ) depends only by the current state ( $X_{t}$ ) and not by the previous ones $X_{t-1}, X_{t-2}, \ldots, X_{1}$.

\begin{equation}
Pr\left(X_{t+1}=x_{t+1}\left|X_{1}=x_{1},X_{2}=x_{2,}...,X_{t}=x_{t}\right.\right)=Pr\left(X_{t+1}=x_{t+1}\left|X_{t}=x_{t}\right.\right).
\label{eq:markovProp}
\end{equation}


The set of possible states $S=\left\{ s_{1},s_{2},...,s_{r}\right\}$ of $X_{j}$
is named the state space of the chain and in the case of DTMC  $S$ is
finite or countable.

A Markow chain is time-homogeneous if the property shown in Equation~\ref{eq:mcHom}
holds. It implies no change in the underlying transition probabilities as time
goes on.
\begin{equation}
Pr\left(X_{t+1}=x\left|X_{t}=y\right.\right)=Pr\left(X_{t}=x\left|X_{t-1}=y\right.\right),
\label{eq:mcHom}
\end{equation}
...
and \\ $p_{ij}^{(n)}=Pr\left(X_{t+k}=s_{j}\left|X_{t}=s_{i}\right.\right)$,
where $k>0$.

The probability distributions of transitions from one state to another
can be expressed by a transition matrix $P$, where each element
...
DTMC are explained in most stochastic processes theory books, see for example \cite{ching2006markov}. 
Valuable references online available are:
\cite{konstantopoulos2009markov}, \cite{probBook}, \cite{wiki:markov} and \cite{bardPpt}.


\subsection{Properties and classification of states}

A state $s_{j}$ is said "accessible" (or "reacheable") from state $s_{i}$ (written $s_{i}\rightarrow s_{j}$) if a system started in state $s_{i}$ has
a positive probability of transitioning into state $s_{j}$ at a certain
point. If both $s_{i}\rightarrow s_{j}$ and $s_{j}\rightarrow s_{i}$, then
$s_{i}$ and $s_{j}$ are said to "communicate".

A "communicating class" is defined to be a set of states that communicate with each other. 
A DTMC can be composed by one or more communicating classes.  If the DTCM is
composed by only one communicating class (if all states in the chain communicate to each other), 
...
A state is said "periodic" if it can only return to itself after a fixed number of transitions greater than 1 (or multiple of a fixed number), else it is called "aperiodic". A state $s_{i}$ is said to be "transient" if, given that we start in state $s_{i}$, there is a positive probability that we will never
...
block form, where the closed comminicating classes are shown at the beginning of the matrix diagonal.
...
is the eigenvector of $P$ such that $\sum_{i=1}^{k}z_{i}=1$, then
...
It is possible to analyze timing of a state being reached. The first passage time from state $s_{i}$ to state $s_{j}$ is the number $T_{ij}$ of steps taken by the chain until it arrives for the first time at state $s_{j}$ given that $X_{0} = s_{i}$. Its probability distribution is defined by Equation~\ref{eq:fpt1}

\begin{equation}
{h_{ij}}^{\left( n \right)} = P\left( {{T_{ij}} = n} \right) = P\left( {{X_n} = s_{j},{X_{n - 1}} \ne s_{j}, \ldots ,{X_1} \ne s_{j}|{X_0} = s_{i}} \right)
\label{eq:fpt1}
\end{equation}

and it can be found recursively using Equation~\ref{eq:ftp2}, knowing that ${h_{ij}}^{\left( n \right)} = p_{ij}$.
...
Consider the following numerical example. Suppose we have a DTMC with a set of 3 possible states $s_{1}$, $s_{2}$ and $s_{3}$.
...
state is $X_{0}=s_{2}$, then the probability vectors $x^{(1)}$ and $x^{(1)}$ can be computed as in Equation~\ref{eq:trPropExEx2} and
Equation~\ref{eq:trPropExEx3}.
...
the implemented \code{initialize} S4 method that assures:
...
%  $\^$ & Integer power of a markov chain.\\
  ...
  \code{as} & Operator to convert from \code{markovchain} objects toward \code{data.frame} and\\
  & \code{table} object.\\
...
\code{show} & \code{show} method for \code{markovchain} objects.\\
  ...
  \code{t} & Transposition operator (it switches byrow slot value and modifies the \\
 &  transition matrix coherently).\\
  \hline
\end{tabular}
...
Operations on markovchain objects can be easily performed.
Using the previously defined matrix we can find what is the probability distribution of expected weather 
states two and  seven days after, given actual state to be cloudy. 
...
column. A column - defined probability matrix could be set up either creating a new matrix or 
transposing an existing \code{markovchain} object thanks to the \code{t} method.
...
Basic informational methods have been defined for \code{markovchain} objects to
quickly get defined states and transition matrix dimension.
...
The \code{markovchain} object's underlying transition Matrix can be displayed using \code{print} or \code{show} 
methods (the latter being less laconic). Similarly, the underlying transition probability diagram 
can be plotted by the use of \code{plot} method ( as shown in Figure~\ref{fig:mcPlot} ) that 
was based on \pkg{igraph} package \citep{pkg:igraph} used to manage and analyze networks data. 
...
Additional parameters can be passed by means of \code{...} to control the network graph layout.
...
Import and export from some specific classes is possible as
Table~\ref{tab:fromAndTo} recapitulates and code below shows.\\
...
\begin{table}[h]
  ...
  \code{matrix} & only to matrix\\
  \code{table} & only from table\\
  \hline
\end{tabular}
\caption{\pkg{markovchain} methods for import and export.}
\label{tab:fromAndTo}
\end{table}  
...
Finally, Table~\ref{tab:demos} lists the demos bundled in the package's demo directory.
...
The \pkg{markovchain} package contains functions to analyze DTMC from a
...
such methods come from \proglang{Matlab} listings
that have been ported into \proglang{R}. For a full description of the undelying
...
\begin{table}[h]
  ...
  Method & Purpose \\
    \hline  \hline
  \code{absorbingStates} & it returns the absorbing states of the transition
  matrix, if any.\\
\code{conditionalDistribution} & it returns the conditional distribution of the
subsequent state $s_{j}$, given actual state $s_{i}$.\\
\code{canonicForm} & it turns the transition matrix into canonic form.\\
\code{is.accessible} & it verifies if a state j is reachable from state i.\\
\code{is.irreducible} & it verifies whether a DTMC is irreducible.\\
\code{period} & it returns the period of an irreducible DTMC.\\
\code{steadyStates} & it returns the vector(s) of steady state(s) in matricial
form.\\
\code{summary} & it summarizes the statistical probabilities of a Markov
chain.\\
...
It is possible for a Markov chain to have more than one stationary distribuition, as the gambler ruin example shows.
...
The key function used within \cite{renaldoMatlab} (and \pkg{markovchains}
...
The function \code{is.accessible} permits to investigate whether a state $s_j$ is accessible from state $s_i$, 
that is whether the probability to eventually reach $s_j$ starting from $s_i$ is greater than zero.
...
In Section~\ref{mathematic} we observed that, if a DTMC is
irreducible, all its states share the same periodicity and this periodicity is also defined as the periodicity of the chain.
The \code{period} function returns the periodicity of the DTMC, provided that it
is irreducible. The example that follows shows how to find if a DTMC is
reducible or irreducible by means of the function \code{is.irreducible} and, in the latter
case, its periodicity.\\
...
The example Markov chain found in \proglang{Mathematica} web site \cite{mathematica9MarkovChain} has 
...
It is therefore easy to compute the probability for the first rainy day to be the third one, given that the current state is sunny.
...
Table~\ref{tab:funs4Stats} lists functions and methods implemented within
the package which help to fit, simulate and predict Markov chains in the
discrete time.
...
function \code{rmarkovchain}. E.g., the following code generates a year
of weather states according to \code{mcWeather} underlying markovian stochastic
process.
...
$p_{ij}$ entry, where the $n_{ij}$ element consists in the number sequences $\left( X_{t}=s_i, X_{t+1}=s_j\right)$
found in the sample
...
An issue occurs when the sample cointains only one realization of a state (say $X_{\beta}$) that is located at the end of the data sequence (thanks
...
$n$-step forward predictions can be obtained using the \code{predict} methods
explicitely written for \code{markovchain} and \code{markovchainList} objects. 
The prediction is the mode of the conditional distribution of $X_{t+1}$ given
$X_{t}=s$, being $s$ the last realization of the DTMC (homogeneous
or non-homogeneous).\\

\subsubsection{Predicting from a markovchain object}

3-days forward predictions from \code{markovchain} object can be generated as
...
This section shows applications of \pkg{markovchain} functions in various
scientifical fields: weather forecasting, Section~\ref{app:weather}, Finance and
Economics, Section~\ref{app:fin}, Actuarial Science, Section~\ref{app:act},
Sociology~\ref{app:sociology}, Genetics and Medicine, Section~\ref{sec:gen}.
...
Section~\ref{sec:wfLandOfOz}, taken from \cite{landOfOz}, the second,
"Alofi Island Rainfall" in Section~\ref{sec:wfAlofi}, is taken from \cite{averyHenderson}.

\subsubsection{Land of Oz}\label{sec:wfLandOfOz}

According to the example by~\ref{sec:wfLandOfOz}, the Land of Oz is 
...
As can be seen from $w_{1}$, if in the Land of Oz today is a nice day, tomorrow it will rain or snow with probability 1. One week later, furtherly, the prediction is
...
Note that, from the seventh day on, the predicted probabilities are substantially equal to the steady state of the chain and don't depend from the starting point. In fact, if we start from a rainy or a snowy day we equally get
...
classified into three states: "0" (no rain), "1-5" (from non zero until 5 mm) and "6+" (more than 5mm). Corresponding dataset is provided within the
...
Other relevant applications of DTMC can be found in Finance, e.g. to model
credit ratings transition (see Section~\ref{fin:fin}) or Economics, e.g. to
perform economic analysis of dynamic systems (see Section~\ref{fin:ec}).
...
carring Standard\&Poor's published data.
...
Therefore, it is easy to convert such matrices into \code{markovchain} objects and to perform some analyses
...
Let the monetary amount of being in a particular state be represented as a m-dimensional column vector $c_{s}$, whilst let the monetary amount of a transition be embodied in a $C^{R}$ matrix in which each component specifies the monetary amount of going from state i to state j in a single step. Henceforth, Equation~\ref{eq:cost} represents the monetary of being in state $i$.
...
and it tipically depends on vehicle, territory, the demographic profile of the policyholder, and policy coverages dept (deductible and policy limits).\\
...
Assuming that the a-priori claim frequency per car-year is 0.05 in the class (being the class the group of policyholders that share the same common characteristics), the underlying BM transition matrix and its underlying steady state are
...
The first example shows the pricing and reserving of a (simple) health insurance contract. The second example analyzes the evolution of a MTPL portfolio characterized by Bonus Malus experience rating feature.
...
Markov chains have been actively used to model progressions and regressions between social classes. The first study was performed \cite{glassHall}, whilst a more recent application can be found in \cite{blandenEtAlii}. The table that follows shows the income quartile of the father when the son was 16 (in 1984) and the income quartile of the son when aged 30 (in 2000), for the 1970 cohort.
...
The \pkg{markovchain} package has been designed in order to provide easily handling of DTMC and to be enough 
flexible to communicate with other packages that implement models based on DTMC.\\

Some numerical issues have been found when working with matricial algebra using
...
Some temporary workaround have been implemented. For example, the condition row/column sums to be equal to
one is valid up to fifth decimal. Similarly, when extracting the eigenvectors
only the real part is taken.\\

Such limitations are expected to be overcomed in future releases. Similarly,
future versions of the package are expected to improve the code in terms of numerical accuracy and rapidity.
More deep internal function profiling and integration of \proglang{C++} code by means of \pkg{Rcpp} package \citep{RcppR} will be explored.


\section{Aknowledgments}*\label{sec:aknowledgements}

The authors wish to thank Michael Cole and Tobi Gutman for their suggestions and bug checks.
